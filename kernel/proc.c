...
// ! resources to reclaim: pid,user page table and corresponding phisical pages,
// ! kernel page table,kernel stack,trapframe
  ...
  for (;;) {
    ...
    for (p = proc; p < &proc[NPROC]; p++) {
      ...
      if (p->state == RUNNABLE) {
        ...
        // * switch to per-process kernel page table.
